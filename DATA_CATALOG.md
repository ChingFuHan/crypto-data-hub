...
**4 datasets** registered in `dataset_registry.json`.
...
- **Known Issues:** Lifecycle remains `draft` and `contract_validated = false`. Raw archive inventories are local and uncommitted. Parquet materialization currently covers `1d` and `4h`; remaining intervals are future work.
- **Status:** draft
