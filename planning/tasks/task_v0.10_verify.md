# task_v0.10_verify.md

# Verify Phase 9 — Binance UM Kline 15M Parquet Materialization

你是本專案的 Verification Agent。

任務：驗證 Phase 9 的 15M Parquet 是否真的可用。
...
Regression：
    ...
    1D / 4H / 1H parquet validation pass
...
Raw manifest：
    ...
    p = Path("local_data/binance_um_klines/interval=15m/manifests/manifest.json")
...
Materialization manifest：
    ...
    raw_p = Path("local_data/binance_um_klines/interval=15m/manifests/manifest.json")
    mat_p = Path("local_data/binance_um_klines/interval=15m/parquet/manifests/materialization_manifest.json")
...
執行：
    ...
    manifest = Path("local_data/binance_um_klines/interval=15m/parquet/manifests/materialization_manifest.json")
    mat = json.loads(manifest.read_text())

    q15 = f"read_parquet('{root_15m}/**/*.parquet', hive_partitioning=true)"
    q1h = f"read_parquet('{root_1h}/**/*.parquet', hive_partitioning=true)"

    actual_files = len(list(Path(root_15m).rglob("*.parquet")))
    actual_rows = duckdb.sql(f"SELECT COUNT(*) FROM {q15}").fetchone()[0]
    rows_1h = duckdb.sql(f"SELECT COUNT(*) FROM {q1h}").fetchone()[0]
    ratio = actual_rows / rows_1h if rows_1h else None

    checks = {
        "manifest_file_match": actual_files == mat.get("file_count"),
        "manifest_row_match": actual_rows == mat.get("row_count"),
        "rows_gt_1h": actual_rows > rows_1h,
        "ratio_ge_3_5": ratio is not None and ratio >= 3.5,
        "bad_null_rows": duckdb.sql(f"""
            SELECT COUNT(*) FROM {q15}
        ...
        """).fetchone()[0],
        "symbol_date_rows_gt_96": duckdb.sql(f"""
            SELECT COUNT(*) FROM (
        ...
        "bad_alignment_rows": duckdb.sql(f"""
            SELECT COUNT(*) FROM {q15}
            WHERE open_time % 900000 != 0
        """).fetchone()[0],
        "bad_close_time_rows": duckdb.sql(f"""
            SELECT COUNT(*) FROM {q15}
            WHERE close_time != open_time + 899999
        """).fetchone()[0],
        ...
        """).fetchone()[0],
        "persistent_csv_count": len(list(Path(root_15m).rglob("*.csv"))),
    }
...
Smoke query：
    ...
    root = "local_data/binance_um_klines/interval=15m/parquet"
...
執行：
    ...
    grep -RIn "15m" docs DATA_CATALOG.md dataset_registry.json 2>/dev/null | head -50
    ls -lh planning/tasks/task_v0.10.md planning/tasks/task_v0.10_verify.md

通過標準：

    docs 有 15M Parquet 使用說明
    DATA_CATALOG.md 有 15M materialized layer
    dataset_registry.json 有 market.binance.um.klines.15m.parquet
    task files 存在
...
ACCEPT 條件：
    ...
    explicit 1D / 4H / 1H regression 通過
    validation --all 通過
    unit tests 通過
    git tracked local_data count = 0
    working tree clean
